	...
	public void saveReports(String date) {
		if (ChocAnSystem.status.checkMode() == false) {
			...
			try {
				...
				writer.println("Provider Number: "
						+ String.format("%09d", record.getProviderNumber()));
				...
				for (int i = 0; i < ChocAnSystem.SRC.size(); i++) {
					...
					if (sRecord.getProviderNumber() == record
						...
						writer.println("Member Name:     "
								+ ChocAnSystem.MRC
										.getSpecificRecordByMemberNumber(
												sRecord.getMemberNumber())
										.getName());
						writer.println("Member Number:   "
								+ String.format("%09d",
										sRecord.getMemberNumber()));
						writer.println("Service Code:    "
								+ String.format("%06d",
										sRecord.getServiceCode()));
						writer.println("Service Fee:     "
								+ ChocAnSystem.PDC
										.getSpecificRecordByServiceNumber(
												sRecord.getServiceCode())
										.get_Cost());
						fee += ChocAnSystem.PDC
								.getSpecificRecordByServiceNumber(
										sRecord.getServiceCode()).get_Cost();
						services += 1;
					}
				}
