		...
		try {
			...
			for (int i = 0; i < ChocAnSystem.SRC.size(); i++) {
				...
				if (sRecord.getMemberNumber() == record.getMemberNumber()) {
					...
					writer.println("Provider Name: " + ChocAnSystem.PRC
							.getSpecificRecordByProviderNumber(sRecord.getProviderNumber()).getName());
					writer.println("Service Name:  " + ChocAnSystem.PDC
							.getSpecificRecordByServiceNumber(sRecord.getServiceCode()).get_Service());
				}
			}
			writer.close();
